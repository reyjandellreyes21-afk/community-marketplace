...
function mobileIconTabClass(active) {
  const base =
    "relative flex min-h-[var(--ui-touch-target,44px)] min-w-0 flex-1 touch-manipulation flex-col items-center justify-center gap-0.5 rounded-xl px-1 py-1.5 transition motion-reduce:transition-none focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-brand-primary/40 dark:focus-visible:ring-brand-accent/40 select-none";
  if (!active) {
    return `${base} text-neutral-400 hover:bg-neutral-100/80 hover:text-neutral-600 dark:text-slate-500 dark:hover:bg-white/[0.06] dark:hover:text-slate-300 [&_.mobile-nav-tab-label]:font-medium`;
  }
  return `${base} bg-brand-primary/12 text-brand-primary dark:bg-brand-accent/15 dark:text-brand-accent [&_.mobile-nav-tab-icon]:text-brand-primary dark:[&_.mobile-nav-tab-icon]:text-brand-accent [&_.mobile-nav-tab-label]:font-semibold`;
}

const mobileNavTabLabel =
  "mobile-nav-tab-label block max-w-full truncate text-[10.5px] leading-tight tracking-tight min-[360px]:text-[11px]";
...
export function LoggedInHeader({
  ...
  /** Primary + secondary navigation stay pinned at all times — scroll-collapse is disabled across views. */
  const mobileShopBrowseScrollCollapseActive = false;
  ...
  const mobileCartZoneTabActive = activeView === VIEWS.CART;
  const mobileAlertsZoneTabActive = activeView === VIEWS.NOTIFICATIONS;
  /**
   * Mobile: hide top utility row (burger, logo, favorites, messages) in bottom-tab zones (cart, orders,
   * alerts, profile) or when parent sets `hideNavigationChrome` (e.g. full-screen product). Footer tabs stay.
   */
  const hideMobilePrimaryRow =
    hideNavigationChrome ||
    mobileCartZoneTabActive ||
    mobileActivityZoneTabActive ||
    mobileAlertsZoneTabActive ||
    mobileProfileZoneTabActive;
  ...
  return (
    ...
    >
      ...
      */}
      ...
      {!hideNavigationChrome ? (
      <nav
        className="mobile-app-secondary-nav md:hidden shrink-0 pb-[env(safe-area-inset-bottom,0px)]"
        role="navigation"
      ...
      >
          ...
          >
            ...
            >
              ...
              <span className={mobileNavTabLabel}>Home</span>
            </button>
            <button
              type="button"
              role="tab"
            ...
            >
              ...
              <span className={mobileNavTabLabel}>Cart</span>
            </button>
            <button
              type="button"
              role="tab"
            ...
            >
              ...
              <span className={mobileNavTabLabel}>Orders</span>
            </button>
            <button
              type="button"
              role="tab"
            ...
            >
              ...
              <span className={mobileNavTabLabel}>Alerts</span>
            </button>
            <button
              type="button"
              role="tab"
            ...
            >
              ...
              <span className={mobileNavTabLabel}>Profile</span>
            </button>
          </div>
        </nav>
      ) : null}
