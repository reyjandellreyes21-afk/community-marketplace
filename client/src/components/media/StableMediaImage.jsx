...
import { focalRectToObjectPosition, normalizeListingFocalRect } from "../../lib/listingImageFocal.js";
...
export function StableMediaImage({
  ...
  objectFit = "cover",
  /** Square thumbnail framing metadata (`cropLeft`, `cropTop`, `cropSize`). Used with `object-fit: cover` only. */
  focalRect = null,
  ...
  const normalizedFocal = normalizeListingFocalRect(focalRect);

  useEffect(() => {
    setLoaded(false);
    setError(false);
    setObjectPosition("");
  }, [trimmed, normalizedFocal?.cropLeft, normalizedFocal?.cropTop, normalizedFocal?.cropSize]);

  const applyFocalFromImage = useCallback(
    (imgEl) => {
      if (!normalizedFocal || objectFit === "contain") {
        setObjectPosition("");
        return;
      }
      const w = Math.max(1, Number(imgEl?.naturalWidth || 0));
      const h = Math.max(1, Number(imgEl?.naturalHeight || 0));
      const pos = focalRectToObjectPosition(normalizedFocal, w, h);
      setObjectPosition(pos || "");
    },
    [normalizedFocal, objectFit],
  );
  ...
  useEffect(() => {
    ...
    if (el?.complete && el.naturalWidth > 0) {
      applyFocalFromImage(el);
      setLoaded(true);
    }
  }, [hasSrc, trimmed, applyFocalFromImage]);
      ...
      {showLiveImg ? (
        <>
          ...
          <img
            ...
            alt={alt}
            onLoad={(e) => {
              applyFocalFromImage(e.currentTarget);
              setLoaded(true);
            }}
            onError={() => {
              setError(true);
