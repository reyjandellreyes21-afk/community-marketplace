...
/** @param {Record<string, unknown>} o @param {Record<string, unknown>} L */
function orderRowVerticalId(o, L) {
  return String(L.verticalId ?? L.categories ?? o.listingVerticalId ?? o.listingCategories ?? o.listing_categories ?? "").trim();
}

/** @param {Record<string, unknown>} o @param {Record<string, unknown>} L */
function orderRowSubId(o, L) {
  return String(L.subId ?? o.listingSubId ?? o.listing_sub_id ?? "").trim();
}

/**
 * One-line category for commerce order rows (buying, selling, booking, commerce-all).
 * Bookings use the service upload category (e.g. Transport Services). Product orders use browse vertical/sub (e.g. Food).
 * @param {unknown} order
 * @param {Record<string, unknown>|null|undefined} cardListing
 * @param {Record<string, unknown>|null|undefined} [listingByListingId]
 * @returns {string}
 */
export function getOrderCommerceRowTypeLabel(order, cardListing, listingByListingId) {
  const o = order && typeof order === "object" ? /** @type {Record<string, unknown>} */ (order) : {};
  const L = cardListing && typeof cardListing === "object" ? /** @type {Record<string, unknown>} */ (cardListing) : {};

  if (orderIsServiceListingBooking(o, listingByListingId)) {
    const meta =
      L.serviceMeta && typeof L.serviceMeta === "object" && !Array.isArray(L.serviceMeta)
        ? /** @type {Record<string, unknown>} */ (L.serviceMeta)
        : null;
    let label = getServiceCategoryDisplayLabel(L, meta);
    if (label) return label;
    const vid = orderRowVerticalId(o, L) || "services";
    const sid = orderRowSubId(o, L);
    label = getServiceCategoryDisplayLabel({ verticalId: vid, subId: sid }, null);
    return label || "Service";
  }

  const vid = orderRowVerticalId(o, L);
  if (!vid) return "";
  return getListingCategoryShortLabel(vid, orderRowSubId(o, L));
}
